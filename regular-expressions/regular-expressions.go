...
func main() {
	...
	// The Submatch variants include information about both the whole-pattern matches and the submatches within those
	// matches. For example this will return information for both p([a-z]+)ch and ([a-z]+).
	...
	// The All variants of these functions apply to all matches in the input, not just the first. For example to find
	// all matches for a regexp.
	...
	// Our examples above had string arguments and used names like MatchString. We can also provide []byte arguments
	// and drop String from the function name.

	fmt.Println(r.Match([]byte("peach")))

	// When creating constants with regular expressions you can use the MustCompile variation of Compile. A plain
	// Compile won’t work for constants because it has 2 return values.
