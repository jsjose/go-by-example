...
func main() {
	...
	// The JSON package can automatically encode your custom data types. It will only include exported fields in
	// the encoded output and will by default use those names as the JSON keys.
	...
	// of response2 above to see an example of such tags.
	...
	// In order to use the values in the decoded map, we’ll need to cast them to their appropriate type. For
	// example here we cast the value in num to the expected float64 type.
	...
	// In the examples above we always used bytes and strings as intermediates between the data and JSON
	// representation on standard out. We can also stream JSON encodings directly to os.Writers like os.Stdout or
