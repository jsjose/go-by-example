...
func main() {
	...
	// Here’s a non-blocking receive. If a value is available on messages then select will take the <-messages case
	...
	// no buffer and there is no receiver. Therefore the default case is selected.
		...
		// We can use multiple cases above the default clause to implement a multi-way non-blocking select. Here we
